...
static Validation<webrtc::IceCandidateInterface *>
ICE_CANDIDATE_INTERFACE_FN(const std::string &candidate,
...
FROM_NAPI_IMPL(std::shared_ptr<webrtc::IceCandidateInterface>, napi_value) {
  return From<webrtc::IceCandidateInterface *>(napi_value)
      .Map([](auto candidate) {
...
TO_NAPI_IMPL(webrtc::IceCandidateInterface *,
  ...
  if (!value->ToString(&candidate_string)) {
    return Validation<Napi::Value>::Invalid(
        "Failed to print the candidate string. This is pretty weird. File a "
        "bug on https://github.com/node-webrtc/node-webrtc");
  }
  ...
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "candidate",
                                        candidate_string)
  ...
  if (mLineIndex < 0) {
    ...
    NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "sdpMLineIndex",
                                          mLineIndex)
  }

  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "foundation",
                                        candidate.foundation())
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "component", component)
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "priority",
                                        candidate.priority())
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "address",
                                        candidate.address().hostname())
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "protocol",
                                        candidate.protocol())
  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "port",
  ...
  if (tcpType.empty()) {
    ...
    NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "tcpType",
                                          candidate.tcptype())
  }
  ...
  if (type == RTCIceCandidateType::kHost) {
    ...
    NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(
        env, object, "relatedAddress", candidate.related_address().hostname())
    NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "relatedPort",
                                          candidate.related_address().port())
  }

  NODE_WEBRTC_CONVERT_AND_SET_OR_RETURN(env, object, "usernameFragment",
                                        candidate.username())
