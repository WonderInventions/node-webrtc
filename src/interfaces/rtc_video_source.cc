...
Napi::Value RTCVideoSource::New(const Napi::CallbackInfo &info) {
  ...
  if (!info.IsConstructCall()) {
    Napi::TypeError::New(env,
                         "Use the new operator to construct an RTCVideoSource.")
  ...
  _source = new rtc::RefCountedObject<RTCVideoTrackSource>(init.isScreencast,
                                                           needsDenoising);
...
Napi::Value RTCVideoSource::GetNeedsDenoising(const Napi::CallbackInfo &info) {
  CONVERT_OR_THROW_AND_RETURN_NAPI(info.Env(), _source->needs_denoising(),
                                   result, Napi::Value)
  return result;
}

Napi::Value RTCVideoSource::GetIsScreencast(const Napi::CallbackInfo &info) {
  CONVERT_OR_THROW_AND_RETURN_NAPI(info.Env(), _source->is_screencast(), result,
                                   Napi::Value)
  return result;
}
...
void RTCVideoSource::Init(Napi::Env env, Napi::Object exports) {
  ...
  Napi::Function func = DefineClass(
      ...
      {InstanceMethod("createTrack", &RTCVideoSource::CreateTrack),
       ...
       InstanceAccessor("needsDenoising", &RTCVideoSource::GetNeedsDenoising,
                        nullptr),
       InstanceAccessor("isScreencast", &RTCVideoSource::GetIsScreencast,
                        nullptr)});
